...
import controllers.game.blocks.RamBlockController;
import controllers.game.blocks.SimpleBlockController;
import exceptions.InvalidMapFileException;
import models.Direction;
import models.game.blocks.AbstractBlockModel;
import models.game.blocks.RamBlock;
import models.game.blocks.SimpleBlock;
import states.GameplayState;
import ui.Breakout;
import util.Utility;
import views.game.blocks.AbstractBlockView;
import views.game.blocks.RamBlockView;
import views.game.blocks.SimpleBlockView;
...
public class MapController {
    ...
    private ArrayList<String> loadMapFromFile(String path) throws InvalidMapFileException, IOException {
        ...
        if (lines.size() != GameParameters.MAP_ROWS)
            throw new InvalidMapFileException("Invalid row count. The read row count was: " + lines.size()); //check row count
    ...
    private SimpleBlockController createController(AbstractBlockModel block) {
        ...
        switch (block.getType()) {
            case SIMPLE:
                return new SimpleBlockController(block.getID() + "Controller");
            case RAM:
                return new RamBlockController(block.getID() + "Controller", ((RamBlock) block).getDirection(), ((RamBlock) block).getDistance());
            default:
                logger.error("Some error occured during the creation of the controller for block: " + block.getID());
                return null;
        }
    }
     ...
     * @return
     */
    private void createView(AbstractBlockModel block) {
